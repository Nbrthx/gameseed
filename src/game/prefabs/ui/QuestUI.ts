...
export class QuestUI extends Phaser.GameObjects.Container {
    ...
    constructor(scene: GameUI) {
        ...
        this.image = scene.add.rectangle(scene.scale.width/2, -200, scene.scale.width/4, 100, 0xeeddaa)
...
class Button extends Phaser.GameObjects.Container{
    ...
    constructor(scene: Phaser.Scene, x: number, y: number, text: string){
        ...
        this.bg = scene.add.rectangle(0, 0, 48, 16, 0xffee88)
    ...
    on(event: string, listener: (...args: any[]) => void){
        ...
        this.bg.on(event, (...args: any[]) => {
            this.bg.setFillStyle(0xaaaaaa);
            setTimeout(() => {

                listener(...args);

                this.bg.setFillStyle(0xffee88);
            }, 100);
        });
